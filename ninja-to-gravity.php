...
// add_action( 'init', function () {
    // $checkbox = GF_Fields::create( [ 'type' => 'checkbox', 'id' => 33, 'formId' => 180 ] );
    // $checkbox->choices = [
    //     [ 'value' => 'abc', 'text' => 'ABC' ]
    // ];

    // $checkbox->sanitize_settings();

    // $output = $checkbox->get_checkbox_choices( '', '', 180 );
...
add_filter( 'gform_export_menu', function ( $setting_tabs ) {
    ...
    if ( GFCommon::current_user_can_any( 'gravityforms_edit_forms' ) ) {
        ...
        if ( GFCommon::current_user_can_any( 'gravityforms_create_form' ) ) {
            ...
            $setting_tabs['40'] = array(
                ...
                'label' => __( 'Import Ninja Form', 'gravityforms' ),
                'icon'  => $icon,
            );
        }
    }
...
add_action( 'gform_export_page_import_ninja_form', function () {
    if ( ! GFCommon::current_user_can_any( 'gravityforms_edit_forms' ) ) {
    ...
    <div class="gform-settings__content">
            <form method="post" enctype="multipart/form-data" class="gform_settings_form">
                ...
                <div class="gform-settings-panel gform-settings-panel--full">
                    <header class="gform-settings-panel__header"><legend class="gform-settings-panel__title"><?php esc_html_e('Import Ninja Forms', 'gravityforms'); ?></legend></header>
                    <div class="gform-settings-panel__content">
                        <div class="gform-settings-description">
							<?php
							echo esc_html__( 'Select the Ninja Forms export files you would like to import. Please make sure your files have the .nff extension, and that they were generated by the Ninja Forms Export form tool. When you click the import button below, Gravity Forms will import the forms.', 'gravityforms' );
							?>
                        </div>
